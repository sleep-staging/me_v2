...
def combine_to_subjects(root_dir, output_dir):
    ...
    for i in files_dict:
        if len(files_dict[i]) == 2:
            ...
            x1_mean,x1_std = np.mean(x1.reshape(-1,2),axis=0).reshape(1,1,2), np.std(x1.reshape(-1,2),axis=0).reshape(1,1,2)
            x1 = (x1-x1_mean)/x1_std
            x2 = np.load(files_dict[i][1])["x"]
            x2_mean,x2_std = np.mean(x2.reshape(-1,2),axis=0).reshape(1,1,2), np.std(x2.reshape(-1,2),axis=0).reshape(1,1,2)
            ...
            new_x_mean,new_x_std = np.mean(new_x.reshape(-1,2),axis=0).reshape(1,1,2), np.std(new_x.reshape(-1,2),axis=0).reshape(1,1,2)
...
def main():
    ...
    subjects_output_dir = "/scratch/SLEEP_data/numpy_norm_subjects"
